...
<div style="flex: 1 1 30px;">
    ...
    <small id="remaining_weight_display" class="form-text text-danger fw-bold"
        style="font-size: 1.1rem; display: inline-block; width: 100px;"> <!-- reduced width -->
        BW: 0.00 kg
    </small>
</div>
...
<script>
    ...
    const grnCode = grnSelect.value;

    if (!grnCode) return;

    fetch(`https://wday.lk/AA/sms/grn-entry/${grnCode}/remaining`)
        .then(response => response.json())
        .then(data => {
            let remainingPacks = data.packs;
            let remainingWeight = data.weight;

            remainingPacksDisplay.textContent = `BP: ${remainingPacks}`;
            remainingWeightDisplay.textContent = `BW: ${remainingWeight.toFixed(2)}`;
        })
        .catch(err => console.error('Error fetching GRN remaining stock:', err));
}
...
function sendReceiptEmail(html, customerName, recipientEmails) {
    return fetch('{{ route('send.receipt.email') }}', {
        ...
        body: JSON.stringify({ receipt_html: html, customer_name: customerName, emails: recipientEmails })
    })
    .then(res => res.json())
    .then(data => {
...
// Core F1 print function (Print + Email)
...
async function handleF5() {
    ...
    try {
        ...
        const receiptHtml = `<div class="receipt-container" style="margin-left:-8px;margin-right:5px;">
                ...
                <div style="text-align:left;margin-bottom:5px;">
                    <table style="width:100%;font-size:10px;border-collapse:collapse;">
                        ...
                        <tr>
                            <td colspan="2"></td>
                            <td colspan="2" style="text-align:right;"><strong>${customerName.toUpperCase()}</strong></td>
                        </tr>
                    </table>
                </div>
                <hr>
        ...
        // Only email (no print)
        sendReceiptEmail(receiptHtml, customerName, recipientEmails);
        ...
        if (result.success && result.sale) {
            ...
            if (updatedIndex !== -1) {
                ...
                fetchSalesData().then(() => {
                    ...
                    if (refreshedSale) {
                        populateFormForEdit(refreshedSale);
                    }
                });
            }
